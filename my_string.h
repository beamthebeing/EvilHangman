...
object will be set to be one greater that is required to hold the string. 
...
are the same and returns a number greater than zero if the string represented
by hLeft_string is bigger than hRight_string.
*/
...
3) A stopping condition is met if we read whitespace character after
...
necassarily resize the array unless needed.
 */
...
Gauranteed to not cause a resize operation of the internal data. Returns
...
Postconditon: Returns an enumerated with value TRUE if the string
